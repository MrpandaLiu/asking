  ...
  <div class="myask flex-column">
    ...
    <div class="ask-list flex-column">
      ...
      >
        ...
        <div class="question-intro">{{item.questionContent}}</div>
        <div class="question-info">
          <span>{{item.questionView}} 浏览</span> ·
          <span>{{item.commentCount}} 评论</span> ·
          <span style="color: red;" @click="deleteAsk(item.questionId)">删除</span>
        </div>
      </div>
      <div
      ...
      >
        <div>
          <h3>
            {{box.boxTitle}}
            <van-button
              @click="copyInfo(box.boxTitle,box.boxId,box.boxPassword)"
              size="mini"
              round
            >分享</van-button>
          </h3>
        </div>
        <div class="time">{{box.boxTime}}</div>
        <div class="time">
          <strong>Id：</strong>
          {{box.boxId}}
        </div>
        ...
        <div class="question-intro">{{box.boxContent}}</div>
        <div class="question-info">
          <span>{{box.boxCount}} 评论</span> ·
          <span style="color: red;" @click="deleteB(box.boxId)">删除</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    copyInfo(title, id, pwd) {
      ...
      let mydata = `我的问题是“${title}”,请打开网址：http://39.106.58.159/#/answerbox/${id},输入密钥：${pwd}后即可回答`;
      _input.value = mydata;
